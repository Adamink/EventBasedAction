...
from utils.losses import mse2D, mpjpe, get_indices, mse2D_numpy, mpjpe_numpy
...
def main(gpus, cfg):
    ...
    best_metric, best_epoch = train(
     ...
     scheduler, criterion, metric, logger, board)
    logger("Best Epoch {:3d} test_metric: {:.2f}".format(best_epoch, best_metric))
    logger.close()
